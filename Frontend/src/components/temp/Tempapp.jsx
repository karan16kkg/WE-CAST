...
const Tempapp = () => {
  ...
  // useEffect(() => {
  //   axios.get(`https://api.openweathermap.org/data/2.5/weather?q=chandigarh&appid=a322d39c25aa93e12bfe3ff1a6c68891`)
  //   .then((response)=>{
  //     let x=response.data;
  //     console.log(x);
  //     setcity(x.name);
  ...
  const getdata = ()=>{
    setcity(z);
    console.log(city);
    axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=a322d39c25aa93e12bfe3ff1a6c68891`)
    .then((response)=>{
      let x=response.data;
      console.log(x);
      // setcity(x.name);
    ...
    <>
      ...
      <div id='home' className='h-screen'>
        <div className='mt-14 h-14 items-center stati justify-center flex h-fit'>
          <input value={data.country} className='w-1/3 h-12 text-xl rounded-full px-5 border-2 border-purple-500 outline-none ml-28 mt-8' type="search" name="country" onChange={handleChange} />
